...
void Polynomial::operator = (vector<pair<string, float>> newPoly)
{
  polynomial = newPoly;
}
...
vector<pair<string, float>> Polynomial::getPoly()
{
  return polynomial;
}
...
void Polynomial::addMono(Monomial mono)
{
  ...
  plItr = findItem(polynomial, key);
  cout << prev(plItr)->first << endl;
...
{
  vector<pair<string, float>> resultPoly;
  vector<pair<string, float>>::iterator resultItr;
  ...
  for(mpItr=multiPolynomials.begin(); mpItr!=multiPolynomials.end(); mpItr++)
  {
    ...
    for(itr=poly.begin(); itr!=poly.end(); itr++)
    {
      ...
      resultItr = findItem(resultPoly, key);

      // If found the same key
      // add the value only
      if(resultItr!=resultPoly.end())
      {
        resultItr->second += value;
        continue;
      }

      // or insert new key and value
      resultPoly.push_back(make_pair(key, value));
    }
  }
...
Polynomial MultiPolynomials::operateMultiply(Polynomial poly1, Polynomial poly2)
{
  ...
  vector<pair<string, float>>::iterator polyItr; 
  vector<pair<string, float>>::iterator prevItr; 
  ...
  for(prevItr=prevPoly.begin(); prevItr!=prevPoly.end(); prevItr++)
  {
    ...
    for(polyItr=poly.begin(); polyItr!=poly.end(); polyItr++)
    {
      ...
      resultItr = findItem(resultPoly, newKey);
      ...
      resultPoly.push_back(make_pair(newKey, newValue));
    }
  }
...
vector<pair<string, float>>::iterator findItem(vector<pair<string, float>> vec, string key)
{
  vector<pair<string, float>>::iterator itr;

  for(itr=vec.begin(); itr!=vec.end(); itr++)
  {
    string baseKey = itr->first;
    if(key==baseKey) 
    { 
      return itr;
    }
