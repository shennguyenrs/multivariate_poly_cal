...
{
  private:
    vector<pair<string, float>> polynomial;
    vector<pair<string, float>>::iterator plItr;
    vector<pair<string, float>>::iterator lastItem = polynomial.end();
  ...
  public:
    ...
    void operator = (vector<pair<string, float>> newPoly);
    ...
    // Get polynomial map
    vector<pair<string, float>> getPoly();
...
// Find items in vector
// returns pointer to item position
